...
{
    QPoint currentMousePos = e->pos();
    int dx = currentMousePos.x() - lastMousePressPos.x();
    int dy = currentMousePos.y() - lastMousePressPos.y();

    if ((qxt_d().m_hHeader->offset() >= horizontalScrollBar()->maximum() + qxt_d().m_hHeader->defaultSectionSize() && dx < 0)
            || (qxt_d().m_hHeader->offset() <= -qxt_d().m_hHeader->defaultSectionSize() && dx > 0) || abs(dx) < 20)
    {
        dx = 0;
    }
    if ((qxt_d().m_vHeader->offset() >= verticalScrollBar()->maximum() + qxt_d().m_vHeader->defaultSectionSize() && dy < 0)
            || (qxt_d().m_vHeader->offset() <= -qxt_d().m_vHeader->defaultSectionSize() && dy > 0) || abs(dy) < 20)
    {
        dy = 0;
    }
    ...
    dx = (int)(dx * 0.8) / qxt_d().m_hHeader->defaultSectionSize();// * qxt_d().m_hHeader->defaultSectionSize();
    dy = (int)(dy * 0.4) / qxt_d().m_vHeader->defaultSectionSize();// * qxt_d().m_vHeader->defaultSectionSize();

//    int dxSectionOffset = dx % qxt_d().m_hHeader->defaultSectionSize();
//    if (abs(dxSectionOffset) < (int)(qxt_d().m_hHeader->defaultSectionSize() / 2))
//        dx -= dxSectionOffset;
//    else if (abs(dxSectionOffset) > (int)(qxt_d().m_hHeader->defaultSectionSize() / 2))
//        dx += (dx > 0 ? 1 : -1) * qxt_d().m_hHeader->defaultSectionSize() - dxSectionOffset;

//    int dySectionOffset = dy % qxt_d().m_vHeader->defaultSectionSize();
//    if (abs(dySectionOffset) < (int)(qxt_d().m_vHeader->defaultSectionSize() / 2))
//        dy -= dySectionOffset;
//    else if (abs(dySectionOffset) > (int)(qxt_d().m_vHeader->defaultSectionSize() / 2))
//        dy += (dy > 0 ? 1 : -1) * qxt_d().m_vHeader->defaultSectionSize() - dySectionOffset;
//    qDebug() << dx << dy;
//    scrollContentsBy(dx, dy);
    horizontalScrollBar()->setValue(-dx * qxt_d().m_hHeader->defaultSectionSize() + horizontalScrollBar()->value());
    verticalScrollBar()->setValue(-dy * qxt_d().m_vHeader->defaultSectionSize() + verticalScrollBar()->value());
//    horizontalScrollBar()->setValue(qxt_d().m_hHeader->offset() >= horizontalScrollBar()->maximum()
//                                    ? horizontalScrollBar()->maximum()
//                                    : (qxt_d().m_hHeader->offset() <= 0 ? 0 : qxt_d().m_hHeader->offset()));
//    verticalScrollBar()->setValue(qxt_d().m_vHeader->offset() >= verticalScrollBar()->maximum()
//                                  ? verticalScrollBar()->maximum()
//                                  : (qxt_d().m_vHeader->offset() <= 0 ? 0 : qxt_d().m_vHeader->offset()));
...
void QxtScheduleView::mouseDoubleClickEvent(QMouseEvent *)
{
...
void QxtScheduleView::mouseReleaseEvent(QMouseEvent *)
{
    int dx = (qxt_d().m_hHeader->offset() - horizontalScrollBar()->maximum()) > 0
            ? qxt_d().m_hHeader->offset() - horizontalScrollBar()->maximum()
            : (qxt_d().m_hHeader->offset() < 0 ? qxt_d().m_hHeader->offset() : 0);
    int dy = (qxt_d().m_vHeader->offset() - verticalScrollBar()->maximum()) > 0
            ? qxt_d().m_vHeader->offset() - verticalScrollBar()->maximum()
            : (qxt_d().m_vHeader->offset() < 0 ? qxt_d().m_vHeader->offset() : 0);
    if (dx || dy)
        scrollContentsBy(dx, dy);
...
void QxtScheduleView::wheelEvent(QWheelEvent *)
{
