...
void SqlWorker::createTables() {
    ...
    for (int i = 0; tableDefineArr[i].name != nullptr && tableDefineArr[i].def != nullptr; ++i) {
        if (!db.tables().contains(tableDefineArr[i].name)) {
            ...
            if (0 == strcmp("TEnumText", tableDefineArr[i].name)) {
                db.transaction();       
...
bool SqlWorker::updateUser(const biotech::osla::UserList &userList) {
...

    {
        ...
        for (int i = 0; i < userList.user_size(); i++) {
            DEBUG_LOG("update user info : " << userList.user(i).first_name() << ":" << userList.user(i).last_name()
                                            << userList.user(i).card_serial());
            if ((uint) userList.user(i).status() != biotech::osla::USER_STATUS_ACTIVE) {
                ...
                query.prepare("replace into TUser(ID,Status,FirstName,LastName,"
                                      "Phone,CardSerial,AccountID,IsInBlack, penaltyStopStart, penaltyStopEnd) "
                                      "values(:ID,:Status,:FirstName,:LastName,"
                                      ":Phone,:CardSerial,:AccountID,:IsInBlack, :penaltyStopStart, :penaltyStopEnd)");
                query.bindValue(":ID", (uint) userList.user(i).id());
                query.bindValue(":Status", (uint) userList.user(i).status());
                query.bindValue(":FirstName", QString(userList.user(i).first_name().c_str()));
                query.bindValue(":LastName", QString(userList.user(i).last_name().c_str()));
                query.bindValue(":Phone", QString(userList.user(i).phone().c_str()));
                if (userList.user(i).has_card_serial()) {
                    query.bindValue(":CardSerial", QString(userList.user(i).card_serial().c_str()));
                } else {
                    query.bindValue(":CardSerial", QVariant(QVariant::String));
                }
                if (userList.user(i).has_account_id()) {
                    query.bindValue(":AccountID", (uint) userList.user(i).account_id());
                } else {
                    query.bindValue(":AccountID", QVariant(QVariant::Int));
                }
                query.bindValue(":IsInBlack", userList.user(i).isblack());
                query.bindValue(":penaltyStopStart",
                                (false == userList.user(i).has_penaltystopstart() ? 0 : userList.user(
                                        i).penaltystopstart()));
                query.bindValue(":penaltyStopEnd", (false == userList.user(i).has_penaltystopend() ? 0 : userList.user(
                        i).penaltystopend()));
                if (!query.exec()) {
                    DEBUG_LOG("SqlWorker: Failed to update TUser:" << ConvertQ2CString(query.lastError().text()));
                    db.rollback();
                ...
                DEBUG_LOG("update user id is : " << userList.user(i).id() << ", phone is :"
                                                 << userList.user(i).phone().c_str());
                if ((uint) userList.user(i).id() == adminId) {
                    adminIndex = i;
                    isAdminUpdated = true;
                }
            }
