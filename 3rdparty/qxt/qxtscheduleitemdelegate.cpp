    ...
    for (int iLoop = 0; iLoop < agendaOption->itemGeometries.size();iLoop++)
    {
                ...
                || (*agendaOption->itemPaintCache)[iLoop].height() != agendaOption->itemGeometries[iLoop].height())
        {
            ...
            */
            ...
            //add by dada 2013/04/14
            if (iLoop == 0 && agendaOption->itemHeaderHeight > 0 && agendaOption->itemHeaderHeight <= (rect.height() + 1))
            {
                if (agendaOption->itemHeaderHeight * 2 > (rect.height() + 1)) {
                    QRect contentRect(1, 1, rect.width(), rect.height());
                    paintItemContent(&cachePainter, contentRect, *agendaOption, index);
                } else {
                    QRect contentRect(1, rect.height() / 2 - agendaOption->itemHeaderHeight + 1, rect.width(), agendaOption->itemHeaderHeight);
                    paintItemContent(&cachePainter, contentRect, *agendaOption, index);
                    QRect headerRect(1, rect.height() / 2, rect.width(), agendaOption->itemHeaderHeight);
                    paintItemHeader(&cachePainter, headerRect, *agendaOption, index);
                }
            }
            //end by dada
...
// add by dada
void QxtScheduleItemDelegate::paintItemContent(QPainter *painter, const QRect rect, const QxtStyleOptionScheduleViewItem &option, const QModelIndex &index) const
{
